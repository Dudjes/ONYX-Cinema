...
<head>
    ...
    <!-- Color utility fallbacks (ensure theme shows even if Tailwind custom colors fail) -->
    <style>
        :root {
            --onyx: #0B0B0D;
            --charcoal: #1C1F26;
    ...
    <header class="bg-onyx border-b border-gold/20 sticky top-0 z-50 backdrop-blur-md">
        <div class="container mx-auto px-6 lg:px-12 py-5">
            <div class="flex justify-between items-center">
                ...
                <nav class="hidden lg:flex items-center gap-8">
                    ...
                    <a href=""
                        ...
                        Schedule
                        <span
                            class="absolute bottom-0 left-0 w-0 h-0.5 bg-cyan group-hover:w-full transition-all duration-300 {{ Request::is('schedule*') ? 'w-full bg-gold' : '' }}"></span>
                    </a>

                    <a href=""
                        class="relative text-soft-white hover:text-cyan transition-colors duration-300 group {{ Request::is('about') ? 'text-gold' : '' }}">
                        About
                        <span
                            class="absolute bottom-0 left-0 w-0 h-0.5 bg-cyan group-hover:w-full transition-all duration-300 {{ Request::is('about') ? 'w-full bg-gold' : '' }}"></span>
                    </a>

                    <a href=""
                        class="relative text-soft-white hover:text-cyan transition-colors duration-300 group {{ Request::is('contact') ? 'text-gold' : '' }}">
                        Contact
                        <span
                            class="absolute bottom-0 left-0 w-0 h-0.5 bg-cyan group-hover:w-full transition-all duration-300 {{ Request::is('contact') ? 'w-full bg-gold' : '' }}"></span>
                    </a>
                    ...
                    @auth
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit"
                                class="bg-gold text-onyx px-6 py-2.5 rounded-full font-semibold text-sm hover:-translate-y-0.5 hover:shadow-[0_5px_15px_rgba(212,175,55,0.4),0_0_15px_#00E5FF] transition-all duration-300">
                                Logout
                            </button>
                        </form>
                    @else
                        ...
                        <a href="{{ route('register') }}"
                            class="ml-4 px-6 py-2.5 border border-gold text-gold rounded-full font-semibold text-sm hover:bg-gold/10 transition-all duration-300">
                            Sign Up
                        </a>
                    @endauth
                </nav>
            ...
            <nav id="mobileNav" class="lg:hidden hidden flex-col gap-4 mt-6 pb-4">
                <a href=""
                    class="text-soft-white hover:text-cyan transition-colors {{ Request::is('/') ? 'text-gold' : '' }}">
                    Home
                </a>
                ...
                <a href=""
                    class="text-soft-white hover:text-cyan transition-colors {{ Request::is('about') ? 'text-gold' : '' }}">
                    About
                </a>
                <a href=""
                    class="text-soft-white hover:text-cyan transition-colors {{ Request::is('contact') ? 'text-gold' : '' }}">
                    Contact
                </a>
                ...
                @auth
                    ...
                    <form action="{{ route('logout') }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="bg-gold text-onyx px-6 py-2.5 rounded-full font-semibold text-sm w-full">
                            Logout
                        </button>
                    </form>
                @else
                    <a href="{{ route('login') }}"
                        class="bg-gold text-onyx px-6 py-2.5 rounded-full font-semibold text-sm text-center">
                        Sign In
                    </a>
                    <a href="{{ route('register') }}"
                        class="mt-3 block bg-onyx/20 text-gold px-6 py-2.5 rounded-full font-semibold text-sm text-center">
                        Sign Up
                    </a>
                @endauth
            </nav>
